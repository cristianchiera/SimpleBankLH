...
describe("SimpleBank", function () {
  ...
  describe("Treasury Balance", function () {
    it("Should return the correct treasury balance after withdrawals", async function () {
      const { simpleBank, user1, fee } = await loadFixture(
        deploySimpleBankFixture
      );
      ...
      const depositAmount = hre.ethers.parseEther("1");
      const expectedFee = (depositAmount * BigInt(fee)) / BigInt(10000); // Calcular el fee

      // Depositar y retirar para acumular el fee en la tesorería
      await simpleBank.connect(user1).deposit({ value: depositAmount });
      await simpleBank.connect(user1).withdraw(depositAmount);

      // Verificar que el balance en la tesorería sea igual al fee acumulado
